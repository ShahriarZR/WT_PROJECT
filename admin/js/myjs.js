...
function addAdminRow() {
    var output = document.getElementById("adminAdd");
...
function saveNewAdmin() {
    ...
    // Send data to server
    fetch("../control/add_new_admin_control.php", {
        method: "POST",
        headers: {
            "Content-Type": "application/json"
        },
...
function manageCustomer(searchEmail = "") {
    ...
    fetch("../control/manage_customer_control.php", {
        ...
        .then(data => {
            if (Array.isArray(data) && data.length > 0) {
                var table = "<table border='1' id='customerTable'><tr><th>Serial</th><th>Name</th><th>Email</th><th>Date of Birth</th><th>Gender</th><th>Address</th><th>Password</th><th>Phone</th><th>Profile Image</th><th>Actions</th></tr>";
                data.forEach(function (row) {
                    table += "<tr>" +
                        "<td>" + row.customer_id + "</td>" +
                        "<td contenteditable='false'>" + row.name + "</td>" +
                        "<td contenteditable='false'>" + row.email + "</td>" +
                        "<td contenteditable='false'>" + row.dob + "</td>" +
                        "<td contenteditable='false'>" +
                        "<select disabled>" +
                        "<option value='Male'" + (row.gender === "Male" ? " selected" : "") + ">Male</option>" +
                        "<option value='Female'" + (row.gender === "Female" ? " selected" : "") + ">Female</option>" +
                        ...
                        "<td contenteditable='false'>" + row.c_image + "</td>" +
...
function editCustomerRow(button) {
    ...
    for (var i = 1; i < cells.length - 1; i++) { // Skip Serial and Actions column
        ...
        } else if (i === 3) { // Handling Date of Birth field
            if (!isEditable) {
                let dobValue = cells[i].textContent.trim();
        ...
        } else if (i === 8) { // Handling File Upload for Image
            if (!isEditable) {
                cells[i].innerHTML = "<input type='file' name='c_image' accept='image/*'>";
            } else {
                var fileInput = cells[i].querySelector("input");
                if (fileInput.files.length > 0) {
                    cells[i].dataset.image = fileInput.files[0].name;
                    cells[i].textContent = fileInput.files[0].name;
                } else {
                    cells[i].textContent = cells[i].dataset.image || "";
                }
            }
...
function updateCustomerRow(row) {
    ...
    formData.append("customer_id", cells[0].textContent);
    formData.append("name", cells[1].textContent);
    formData.append("email", cells[2].textContent);
    formData.append("dob", cells[3].querySelector("input") ? cells[3].querySelector("input").value : cells[3].textContent.trim());
    formData.append("gender", cells[4].querySelector("select").value);
    formData.append("address", cells[5].textContent);
    formData.append("password", cells[6].textContent);
    formData.append("phone", cells[7].textContent);

    // Check if an image is selected
    var fileInput = cells[8].querySelector("input");
    if (fileInput && fileInput.files.length > 0) {
        formData.append("c_image", fileInput.files[0]); // Append image file
    }
    ...
    .catch(error => {
        showPopup("Error loading customer data: " + error.message);
    });
}
...
function addCustomerRow() {
    ...
    for (var i = 0; i < 10; i++) {
        ...
        if (i === 3) {
            ...
            cell.innerHTML = "<input type='file' name='c_image' accept='image/*'>";
        } else if (i < 9) {
...
function saveCustomerRow(button) {
    ...
    if (validateCustomerRowData(row)) {
        ...
        var rowData = {
            name: cells[1].textContent,
            email: cells[2].textContent,
            dob: cells[3].querySelector("input") ? cells[3].querySelector("input").value : cells[3].textContent.trim(),
            gender: cells[4].querySelector("select").value,
            address: cells[5].textContent,
            password: cells[6].textContent,
            phone: cells[7].textContent,
            c_image: cells[8].querySelector("input").files.length > 0 ? cells[8].querySelector("input").files[0].name : ""
        };
...
function validateCustomerRowData(row) {
    ...
    var name = cells[1].textContent.trim();
    var email = cells[2].textContent.trim();
    var dob = cells[3].querySelector("input") ? cells[3].querySelector("input").value : cells[3].textContent.trim();
    var gender = cells[4].querySelector("select").value;
    var address = cells[5].textContent.trim();
    var password = cells[6].textContent.trim();
    var phone = cells[7].textContent.trim();
...
function printApproveAccessories(url, outputElement) {
    fetch(url, {
        method: "POST",
        headers: {
            "Content-Type": "application/json"
        ...
        .then(data => {
            if (Array.isArray(data) && data.length > 0) {
                var table = "<table border='1'><tr><th>ID</th><th>Name</th><th>Description</th><th>Price</th><th>Stock</th><th>Seller ID</th><th>Actions</th></tr>";
                data.forEach(package => {
                    table += "<tr>" +
                        "<td>" + package.approve_travel_accessory_id + "</td>" +
                        "<td>" + package.name + "</td>" +
                        "<td>" + package.description + "</td>" +
                        "<td>" + package.price + "</td>" +
                        "<td>" + package.stock + "</td>" +
                        "<td>" + package.seller_id + "</td>" +
                        "<td>" +
                        "<button onclick='approveAccessories(this)'>Approve</button>" +
                        "<button onclick='rejectAccessories(this)'>Reject</button>" +
                        "</td>" +
                        "</tr>";
                });
                table += "</table>";
...
function approveAccessories(button) {
    var row = button.parentElement.parentElement;
    var cells = row.getElementsByTagName("td");
    var rowData = {
        approve_travel_accessory_id: cells[0].textContent,
        name: cells[1].textContent,
        description: cells[2].textContent,
        price: cells[3].textContent,
        stock: cells[4].textContent,
        seller_id: cells[5].textContent
    };
    fetch("../control/insert_travel_accessories.php", {
        method: "POST",
        headers: {
            "Content-Type": "application/json"
        },
        ...
        .then(result => {
            showPopup(result);
            approveNewAccessories();
        })
        .catch(error => {
            showPopup("Error loading admin data: " + error.message);
        });
}
function rejectAccessories(button) {
    var row = button.parentElement.parentElement;
    var cells = row.getElementsByTagName("td");
    var rowData = {
        approve_travel_accessory_id: cells[0].textContent
    ...
    fetch("../control/delete_approve_travel_accessory.php", {
        method: "POST",
        headers: {
            "Content-Type": "application/json"
        },
...
function approveNewAccessories() {
    printApproveAccessories("../control/approve_new_accessories.php", "output");
}
